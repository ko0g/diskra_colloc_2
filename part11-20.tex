...
Пусть $P$ -- ЧУМ, $l$ -- макс. длина цепи. 

Покажем, что $P$ разбивается на $l$ антицепей

Пусть $min \, X = \{x \in X \; | \; \text{$x$ -- мин. в $X$}\}$
...
\vdots
\]
\[P_k = min \, (P \setminus (P_1 \cup \hdots \cup P_{k-1}))\]
\[
\Downarrow
\]
\[
P = P_1 \sqcup \hdots \sqcup P_m
\]
Каждое $P_i$ -- антицепь. Докажем, что $m = l$:

Построим в $P$ цепь размера $m$. Рассмотрим $\forall p_m \in P_m \Rightarrow \exists p_{m-1} \in P_{m-1}: p_m > p_{m-1}$:
\[
p_m > p_{m-1} > \hdots > p_r
\]
\[
min \, (P_m \sqcup \hdots \sqcup P_r \sqcup P_{r-1}) = P_{r-1}
\]
\begin{center}
    $p_r$ не минимальный $\Rightarrow \exists p_{r-1} < p_r$
\end{center}
\[
p_{r-1} \in P_{r-1} \text{ (иначе 2 минимума сравнимы $\Rightarrow$ противоречие)}
\]
\[
\Downarrow
\]
\begin{center}
    Продолжили до конца и получили цепь в $P$ размером $m$
\end{center}
\[
\Downarrow
\]
...
m \leq l
\]
Но мы разбили множество на $m$ антицепей $\Rightarrow l \geq m$
\[
\Downarrow
\]
\[
...
\color{gray}
\subsection*{\textit{Доказательство:}}
Полная индукция по $|P| = n$

Переход ($\forall k < n \rightarrow n$):

Рассмотрим $m = \min(P)$ и удалим его: $P' = P \setminus \{m\}$. Тогда по индукции $P'$ можно разбить на $l$ цепей $\Rightarrow P$ можно разбить на $l$/$l+1$ цепей
1) на $(l+1)$ цепей:

Просто добавим цепь $\{m\}$ и получим $(l+1)$ цепь

2) на $l$ цепей:

Фикс. разбиение $P'$ на $l$ цепей. Пусть $p_i$ -- наименьший элемент из $C_i$ входящий в антицепь $P'$ длиной $l$. Тогда $\{p_1, \hdots, p_l\}$ -- антицепь

От противного, пусть $p_i < p_j$. $p_i$ входил в а/ц $A$ длиной $l$. $A \cap C_j = \{y\}$

$y \geq p_j > p_i \Rightarrow y > p_i \Rightarrow$ противоречие

Так как макс. размер а/ц $l$, $\exists j:$ $m$ сравним с $p_j$ ($p_j > m$).

Рассмотрим цепь $C = \{x \in C_j \; | \; x \geq p_j\} \cup \{m\}$: для $(P \setminus C)$ выполняется предположение индукции: $\max$ размер а/ц $\leq l - 1$ (так как в $(C_j \setminus C)$ нет элементов входящих в а/ц длиной $l$)
\[
\Downarrow
\]
\begin{center}
    $(P \setminus C)$ разбивается на $\leq l-1$ цепей
\end{center}
\[
\Downarrow
\]
\begin{center}
    $P$ разбивается на $\leq l$ цепей
\end{center}
\[
\Downarrow
\]
\begin{center}
    $P$ разбивается на ровно $l$ цепей
\end{center}
}
...
\vspace{-0.5cm}
{
    \color{gray}
\subsection*{\textit{Доказательство:}}
Оценка: $\mathbb{B}^n = B_0 \sqcup \hdots \sqcup B_n \Rightarrow \max |C| \leq n + 1$

Пример: $(0,\hdots,0) < (0,\hdots,1) < \hdots < (1,\hdots,1)$
}
\text{}
\\\\
...
\vspace{-0.5cm}
{
    \color{gray}
\subsection*{\textit{Доказательство:}}
Зафиксируем элемент булевого куба весом $k$. Тогда через него проходит $k!(n-k)!$ цепей макс. длины. Через любые 2 элемента а/ц не может проходить одна и та же цепь, тогда
\[
\sum_{k = 0}^{n} a^k \cdot k! \cdot (n-k)! \leq n!
\]
\[
\sum_{k = 0}^{n} \frac{a^k}{\binom{n}{k}} \leq 1
\]
}
...
По LYM неравенству:
\[
1 \geq \sum_{k = 0}^{n} \frac{a^k}{\binom{n}{k}} \geq \sum_{k = 0}^{n} \frac{a^k}{\binom{n}{n/2}}
\]
\[
\Downarrow
\]
\[
\sum_{k = 0}^{n} a_k \leq \binom{n}{n/2}
\]
Пример: все вершины весом $n/2$. Они несравнимы
}
